...
def fine_tune(text_columns_1, text_columns_2, gold_standard, pq1, pq2, model_name):
...
if __name__ == '__main__':
    ...
    pq1 = pd.read_parquet(f"./data/Abt_e5.pqt")
    pq2 = pd.read_parquet(f"./data/Buy_e5.pqt")
    gold_standard = pd.read_csv(f"{folder}/truth_abt_buy.csv.csv", sep=",", encoding="utf-8", keep_default_na=False)
    model_path = f'{folder}/abt_buy_e5_ft_model'
    fine_tune(["name","description"], ["name", "description"], gold_standard, pq1, pq2, model_name, model_path)

    embedding_model = SentenceTransformer(model_path)
    embed_tranformer.embed(
        input_filename=f'{folder}/Abt.csv',
        text_columns= ["name", "description"],
        prefix="product: ",
        output_filename=f'{folder}/Abt_e5_ft.pqt',
        model=embedding_model
    )


    embed_transformer.embed(
        input_filename=f'{folder}/Buy.csv',
        text_columns= ["name", "description"],
        prefix="product: ",
        output_filename=f'{folder}/Buy_e5_ft.pqt',
        model=embedding_model
    )
