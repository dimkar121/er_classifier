...
name="abt_buy"
files1 =["Abt"] #["votersA"] # ["Scholar"] #["Abt"] #["Scholar"] #["Amazon", "Scholar", "votersA"] #["Scholar"] # ["Amazon", "Scholar", "votersA"]   #"Abt" # "Scholar"  #"Abt" # "ACM" #
files2 = ["Buy"] #["votersB"] #["DBLP2"] # ["Buy"] #["DBLP2"] #["Google", "DBLP2" , "votersB"] #["DBLP2"] #["Google", "DBLP2" , "votersB"]  # "Buy" #"DBLP2" # #"DBLP" #
files3 =["abt_buy"] # ["voters"] #["Scholar_DBLP"] # ["abt_buy"] #["Scholar_DBLP"] #["Amazon_googleProducts", "Scholar_DBLP", "voters"] #["Scholar_DBLP"] #["Amazon_googleProducts", "Scholar_DBLP", "voters"]# "abt_buy" #"Scholar_DBLP" # # "ACM_DBLP" #
id1dfs = ["idAbt"] #["id1"] # ["idScholar"] # ["idAbt"] #["idScholar"] # ["idAmazon", "idScholar", "id1"] #["idScholar"] #["idAmazon", "idScholar", "id1"] #"idAbt" #"idScholar" # "idAbt"  #"idACM" # #gold standard columns
id2dfs = ["idBuy"] #["id2"] # ["idDBLP"]  #["idBuy"] #["idDBLP"] # ["idGoogleBase", "idDBLP", "id2"] #["idDBLP"] #["idGoogleBase", "idDBLP", "id2"] #  "idBuy" # "idDBLP" #"idBuy" #"idDBLP" #
num_candidates = 5
X_data = []
y_data = []
...
for file1, file2, file3, id1df, id2df in zip(files1, files2, files3, id1dfs, id2dfs):
  ...
  for index, row in sampled_gold_standard.iterrows():
      ...
      if id1 in embeddings1 and id2 in embeddings2:
        ...
        #combined_embedding = np.concatenate((np.array(emb1), np.array(emb2), np.array([j_similarity1, j_similarity2, m, price_diff, jw])))
        combined_embedding = np.array([j_similarity1, j_similarity2, m, price_diff, jw])
  ...
  if X.shape[0] == 0:
    print("Error: X_train is empty. Ensure your data preparation step correctly populates X_data.")
    exit()
  if y.shape[0] == 0:
    print("Error: y_train is empty. Ensure your data preparation step correctly populates y_data.")
    exit()
...
if X_train.shape[0] == 0 or X_val.shape[0] == 0:
    print("Error: Training or validation set is empty after split. Ensure you have enough data.")
    exit()
...
# 1. Initialize the model with some standard parameters
xgb_classifier = xgb.XGBClassifier(
    objective='binary:logistic', # for binary classification
    eval_metric='logloss',       # metric to watch for early stopping
    n_estimators=1000,           # a high number of trees, we'll use early stopping
    learning_rate=0.05,
...
xgb_classifier.fit(
    ...
    eval_set=[(X_val, y_val)], # Data to evaluate against for early stopping
    verbose=False # Set to True if you want to see progress
)
xgb_classifier.save_model("./data/abt_buy_xgb_model.json")

# 3. Make predictions on the validation set
print("Evaluating model...")
predictions = xgb_classifier.predict(X_val)

# 4. Check your new performance
